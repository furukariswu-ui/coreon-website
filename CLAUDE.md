...
| 產業場景 › 財會稅務 | `/industries/accounting/` | ✅ 完成，**內容豐富度已大幅擴充**（2026-08-24新增，第2個detail page，同日稍晚追加第二輪內容）。**第一輪**（年度申報/Workflow敘事）：Hero用「2026年度申報總覽」dashboard mockup＋Master flow濃縮成10步驟chip串＋**雙軌並行圖**（客戶生命週期7節點×會計稅務作業週期7節點，「建立服務項目」節點放大強調＋虛線連接兩軌）＋建立服務項目feature＋4大工作週期grid＋深色時間軸（平時/年中/年底/年後/申報期＋engine公式）＋年度作業中心feature（14步驟拆4階段）。**第二輪**（使用者提供兩份完整產品規劃：Coreon Smart Ledger／Coreon Client 360，指示「可以優化內容但不要全部覆蓋」＋「css系統的預覽也要跟金融的一樣有demo」）：新增①**三大支柱總覽區**（Client 360／Smart Ledger／Workflow 3張pillar卡，放在master flow之後、原有雙軌圖之前，把原本內容回溯定位成「Workflow支柱」）②**Smart Ledger完整模組**——intro+pipeline公式、多來源收件+AI辨識feature（發票欄位自動抓取mockup）、稅務分類+智慧稅務工作池feature（326筆自動拆分成6個池的mockup，直接採用使用者給的182/267/12/21/8/5筆示範數字）、Exception First feature（AI已完成判斷94%+待覆核18筆mockup）、紙本正本管理feature（Digital Twin＋Batch QR Code兩個mockup堆疊）、6大核心能力grid、收尾金句③**Client 360完整模組**——深色intro（沿用`#241F1A`，這頁第2個深色區塊）、客戶總覽頁feature（ABC科技完整狀態卡mockup）、Client Tax Center feature（稅務總覽表格mockup）、統一待辦+繳稅追蹤feature（「目前有3件事情需要處理」todo卡mockup，直接採用使用者給的範例）、Manager Dashboard feature（7項全所統計grid，286/42/31/18/46/5/2都是使用者原文數字）、Client Health Score feature（🟡注意客戶vs🔴高風險客戶對照卡）、收尾「不是CRM」金句。原有「真正的問題」5點痛點與CTA保留、移到全部內容最後作收尾。全部沿用`css/coreon-v2.css`共用系統＋finance頁同一套mockup component（`.frame`/`.mpill`/`.tbl`），新增幾個可重用的CSS pattern（`.pillar-grid`三支柱卡、`.stat-grid`統計方塊、`.health-grid`風險對照卡、`.todo-card`統一待辦清單），沒有杜撰任何案例或統計數字，所有mockup示範資料都是使用者原文提供的illustrative demo data。**第三輪**（同日再追加，使用者提供完整的「Coreon Client 360｜產品頁UIUX規劃」，含精確layout比例/逐區文案/design system規範，指示「優化進會計介紹裡面」）：**整段替換掉第二輪做的簡易Client 360模組**，改成貼合新brief的12-section完整版：01 Hero（0.85:1.15雙欄，大型dashboard mockup含統編/主辦人/78%整體進度）02 The Disconnected Client（問題切入，LINE/Excel/紙本/Email/Calendar分散節點匯聚到中心的diagram）03 One Client One Workspace（左側10項sidebar nav＋右側6卡片Overview grid的完整工作台UI）04 Service Progress（Linear風格細線+節點progress，7步驟含done/now/pending三態＋右側Action Panel）05 What Needs Attention（🔴🔵🟡三色left-border待辦列表，這是brief特別強調的UX重點）06 Tax & Payment Center（3個mini-stat+4列稅務表格）07 Client Portal（客戶視角portal mockup，午安問候+進度條+2待辦）08 Client Timeline（歷史時間軸，6筆事件含系統自動/人工操作區分）09 Manager View（**深色區塊搬到這裡**，從單一客戶轉場到全所視角，7項統計+4列客戶列表表格）10 Client Health（升級成具名+分數版：ABC科技82/100健康、DEF設計48/100🔴 Attention Required+4條風險清單）11 System Architecture（Client 360核心+8個環繞領域node+底層Intelligence Layer深色footer）12 Closing（收斂成一段big-statement，不重複CTA按鈕，避免跟頁面最下方的正式CTA重複）。**照著brief的design system建議**：card border-radius維持10-22px（沒有用brief提到要避免的24px超圓角）、shadow沿用既有偏淡的既有慣例、status色彩邏輯沿用橘色系（brief原文建議「品牌藍/紫」代表進行中，但COREON品牌色是橘不是藍紫，判斷應該以實際品牌色為準，沒有照搬字面建議）。**圖片素材**：brief要求2-3張情境照片（Client Portal區、Final CTA區），但沒有實際圖檔也沒有生圖能力，**沒有硬做假照片或佔位空格**，改成brief自己也提到的備案「其他全部用Coreon UI本身當主視覺」——把這兩區也做成強化的UI mockup/漸層處理，維持頁面完整度，不留看起來像未完工的空白佔位區。**發現並修正一個真bug**：新增的2個雙欄grid區塊（Hero、Disconnected Client）跟1個雙欄區塊（Service Progress）原本用行內`style="display:grid;grid-template-columns:..."`直接寫在元素上，這種寫法**沒有響應式斷點**，手機寬度下會維持桌面的雙欄比例，導致其中一欄被擠壓到74px超級窄——已改成共用class（`.split-hero`/`.split-panel`）+ `@media(max-width:860px)`斷點，本機驗證過手機寬度下正確收合成單欄、無橫向溢出。頁面現在有28個section，比第二輪的23個再多5個。**第四輪**（同日再追加）：使用者提供工程師寫的參考檔`會計師事務所管理系統_功能特色介紹.html`（獨立配色系統，藍/綠/琥珀/紅/紫多色，跟COREON品牌不符），先做「先羅列不變動」的比對分析交給使用者確認，使用者回覆「加進去，表格清楚的參考檔案格式，品牌顏色改成COREON的」。**新增內容**（全部套用COREON既有橘色系`.mpill`/status邏輯，不採用來源檔的多色系統）：①核心觀念+比較表（一般做法vs COREON，4列，插入在雙軌圖之後）②法定時程真實日曆（9個具體期限項目+5段式提醒流程，插入在4大工作週期之後）③三大業務工作流——**新增工商登記與簽證業務兩條完全沒做過的服務線**（記帳稅務原本就有）④工時與績效考核——**全新章節**，事務所內部員工管理（4種免摩擦計時方式、7項指標儀表板、職級分級目標、主管±15%調整權、客戶超服務工時獨立分類）⑤一鍵生成報表三方分類（給客戶/給稅局/內部管理）⑥AI輔助誠實分層——**全新章節**，用`.ai-do`/`.ai-not`兩色卡片明確列出「現在就值得做」與「我們不做的事」（不做端到端無人記帳、AI不能生成查核結論），這是全站第一個明講AI能力邊界的區塊⑦資料安全與法遵（深色區塊，這頁第3個深色區，跟finance頁的資安區塊呼應，之前這頁沒有這塊）⑧為什麼不用現成軟體（3張卡：國外事務所軟體點名Karbon/Canopy/TaxDome、國內報稅工具、案件管理型系統）⑨FAQ（9題手風琴，`<details>/<summary>`原生元件）。**強化既有內容**：Tax & Payment Center補上LINE收款自動比對銀行對帳單的機制＋「系統不碰金流、不串接支付API」的範圍聲明；One Client Workspace區塊補上「客戶群組」（老闆名下多家公司歸戶）與「客戶獲利分析」2張小卡。**內容取捨的判斷**：來源檔裡具體到可查證的法規細節（確切罰鍰金額、公報編號、「個資法2025年11月修正」這類特定日期條文）**刻意軟化成較保守的敘述**（例如拿掉具體罰鍰金額改成「逾期恐有罰鍰」、拿掉公報編號改成「依審計準則相關規範」），只保留功能性主張（COREON會自動處理這些計算/提醒），因為我沒有能力逐條查證台灣現行稅法是否正確，這跟頁面上其他純示意的mockup示範數字是不同風險等級的內容——加了一行「節錄，實際期限請以財政部公告為準」的提醒。**沒有搬進來的部分**：來源檔的「12·導入方式」（3期分階段時程，第一期4-5個月/第二期3-4個月/第三期4-6個月）跟FAQ裡對應的「導入要花多久」題目，因為這讀起來更像針對特定客戶的銷售提案／導入時程承諾，不是通用的公開行銷頁內容，沒有不經確認就放上公開網站——如果使用者要放，需要另外討論怎麼呈現比較合適。也把來源檔「律師事務所的系統不適用」的比較點改成更中性的「案件管理型系統」，避免在COREON自己也規劃法律產業頁的情況下，措辭上顯得自相矛盾。頁面現在37個section、1708行，比第三輪的28個section又大幅增加。 |
